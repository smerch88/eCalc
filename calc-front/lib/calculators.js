export function calculateBoilerEnergyConsumption(
  ...
  const energyRequiredKJ =
    massOfWater * specificHeatWater * temperatureDifference;
  ...
  const nightRateCost =
    ((totalCost * nightRateUsagePercentage) / 100) * nightRateFactor;
  ...
  const totalCostWithSubscription =
    (normalRateCost + nightRateCost + coldWaterCost) / 100;
...
export function calculateWMConsumption({
  ...
  let energyPerLoad =
    energyConsumptionPerLoad[efficiencyClass]?.[loadSize] ||
    energyConsumptionPerLoad.A?.medium; // За замовчуванням A/medium
  let waterPerLoad =
    waterUsagePerLoad[efficiencyClass]?.[loadSize] ||
    waterUsagePerLoad.A?.medium; // За замовчуванням A/medium
  ...
  const nightRateEnergyCost =
    totalEnergyCost * nightRateUsagePercentage * nightRateFactor;
  ...
  return {
    yearly: {
      ...
      totalYearlyCost:
        (normalRateEnergyCost + nightRateEnergyCost + totalWaterCost) / 100, // загальна вартість (грн/рік)
    },
    monthly: {
...
export function calculateMWConsumption({
  ...
  const efficiencyFactor =
    ageInYears > 0 ? Math.max(0.9 - ageInYears * 0.02, 0.5) : 1; // Знижка на ефективність (по 2% на рік)
...
export function calculateLightingConsumption({
  ...
  const monthlyEnergyConsumption =
    powerInKW * hoursPerDay * numberOfBulbs * daysPerMonth;
